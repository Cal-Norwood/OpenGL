    ...
    void load_material(const char* filename) {
        ...
        for (string& line : lines) {
            if (line[0] == 'n' && line[1] == 'e') {
                sscanf_s(line.c_str(), "newmtl %s", str);
                material = str;
...
public:
    ...
    void load(const char* filename) {
        ...
        float a, b, c;
        char str[256]; // Ensure this is large enough for your material file names
        ...
        for (std::string& line : lines) {
            ...
            if (line[0] == 'v') {
                ...
                sscanf_s(line.c_str(), "mtllib %255s", str, (unsigned)_countof(str));
                std::string file = prefix + str;
                load_material(file.c_str());
            }
            else if (line[0] == 'u' && line[1] == 's') {
                sscanf_s(line.c_str(), "usemtl %255s", str, (unsigned)_countof(str));
